...
var sgGMaps = function() {
  return {
    ...
    init: function(idCanvas, startPoint) {
      if (GBrowserIsCompatible()) {
        var map = new GMap2(document.getElementById(idCanvas));
	map.setCenter(new GLatLng(startPoint[0], startPoint[1]), sgGMaps._zoom);
        map.setUIToDefault();
	return map;
      }
    },
    ...
    addMarks: function(map, itemList) {
      itemList.map(function(x) {sgGMaps.addMark(map, x['address'], 'click', x['toString'](x));});
    }
  };
}();
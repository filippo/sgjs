...
sg.html.extend('ready',
	       function(fn){
		   if (document.addEventListener) { //check for Mozilla, Opera and Webkit
		       document.addEventListener("DOMContentLoaded", fn, false);
		       // A fallback to window.onload, that will always work
		       window.addEventListener("load", sg.html.ready, false );
		   } else if (document.all && !window.opera) { // test to see if we are on IE
...
/*
 * Get or set the value attribute of an html element.
 */
sg.html.extend('val', 
	       function(el, value) {
		   var elRef = sg.html.byId(el);
...
sg.html._each('a big blockquote br b center code div em form h1 h2 h3 h4 h5 h6 hr img iframe input i li ol option pre p script select small span strong style sub sup table tbody td textarea tr ul u'.split(' '),
    function(label){
        sg.html[label]=function(){
            var tag=document.createElement(label);
            sg.html._each(arguments, function(arg){
                if(arg.nodeType) {
		  tag.appendChild(arg);
